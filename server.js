...
http.createServer(function (request, response) {
        ...
        request.on('end', function () {
            ...
            if(body.id){
                string = "UPDATE clients SET name = '"+body.name+"',surname = '"+body.surname+"',patr = '"+body.patr+"', bday='"+
                    body.bday+"',seria = '"+body.seria+"',num ='"+body.num+"', issuedBy='"+
                    body.issued+"', data='"+ body.data+"',idPasport = '"+body.num_id+"',birthplace ='"+body.place+"', town='"+
                    body.town+"', address='"+ body.address+"',telD = '"+body.td+"',telM ='"+body.tm+"',job='"+body.job+"', post='"+body.post+"', mail='"+
                    body.mail+"', cityOfReg='"+ body.townpr+"',addressOfReg = '"+body.addresspr+"',famStatus ='"+body.fam+"',nation='"+
                    body.nation+"', invalid='"+ body.inval+"', pensioner='"+ body.b+"', income='"+ body.income+"' WHERE id = "+body.id;
            }
            else {
                string = "INSERT clients (name, surname, patr, bday, seria, num, issuedBy, data, idPasport, " +
                    "birthplace, town, address, telD, telM, job, post, mail, cityOfReg, addressOfReg, famStatus, " +
                    "nation, invalid, pensioner, income) " +
                    "VALUES('" + body.name + "','" + body.surname + "','" + body.patr + "','" + body.bday + "','"
                    + body.seria + "','" + body.num + "','" + body.issued + "','" + body.data + "','" + body.num_id + "','"
                    + body.place + "','" + body.town + "','" + body.address + "','" + body.td + "','" + body.tm + "','"+ body.job+"','"+ body.post +"','" + body.mail + "','"
    ...
    if (request.method == 'POST' && request.url == "/edit_clients") {
        var query = connection.query("SELECT * FROM clients", function (error, result) {
            response.end(JSON.stringify(result), 'utf-8');
    ...
    if (filePath == './') {
        filePath = './front-end/Main page.html';
    } else {
